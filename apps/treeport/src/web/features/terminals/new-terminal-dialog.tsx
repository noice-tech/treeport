...
import type { CreateTerminalInput } from './terminal-workspace'
...
export function NewTerminalDialog({
  ...
  const noResults = !showShell && filteredPresets.length === 0
  ...
  return (
    ...
    >
      ...
      >
        <DialogTitle className="sr-only">New terminal</DialogTitle>
        <DialogDescription className="sr-only">
          Search for a shell or preset to start
        ...
        <div className="flex min-w-0 items-center gap-3 border-b border-white/8 px-4">
          ...
          <input
            ...
            autoComplete="off"
            autoFocus
            aria-label="Search terminal actions"
            placeholder={
              worktreeName ? `New terminal in ${worktreeName}` : 'New terminal'
            }
            onChange={(event) => {
              ...
              const actions = Array.from(
                event.currentTarget
                  ...
                  ?.querySelectorAll<HTMLButtonElement>(
                    '[data-terminal-launch]:not(:disabled)'
                  ) ?? []
              )
              if (actions.length === 0) {
                return
