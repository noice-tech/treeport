...
The panel saves the current address in its panel storage. If the target application includes `@treeport/panel-sdk`, the Browser panel also follows and saves client-side navigation, including routes changed by a React router. Browser security prevents observing navigation in a cross-origin target that does not include the SDK. **Reload** affects only the current client. Another client uses the saved address when it later opens or reloads the panel.
...
A target application can use `treeport.panel.setTitle()` to set the panel title. Importing the SDK also lets the Browser panel observe its current route; no route-specific SDK call is required. The target cannot use context, diff, network discovery, storage, shortcuts, or workspace navigation. Add `@treeport/panel-sdk` to the target application's normal build if it uses either integration.
