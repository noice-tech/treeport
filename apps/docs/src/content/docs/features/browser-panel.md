...
A target application can use `treeport.panel.setTitle()` to set the panel title and `treeport.shortcuts.onFind()` to handle `Cmd/Ctrl+F`. The desktop app reserves `Cmd/Ctrl+1` through `Cmd/Ctrl+9` for Treeport workspace selection, including while focus is inside the target application. In a browser client, the target must include the SDK to route these numbered shortcuts to Treeport. Add `@treeport/panel-sdk` to the target application's normal build to use the SDK integrations. Targets cannot use context, diff, network discovery, or storage through the Browser panel.
