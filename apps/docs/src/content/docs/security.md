...
The Browser package can load an arbitrary HTTP or HTTPS site in a nested iframe. The package relays only the target's client-local title and keyboard shortcut messages. The target cannot use context, diff, network discovery, or storage methods. Browser iframe restrictions remain in effect, and Treeport does not bypass a target's framing policy.
