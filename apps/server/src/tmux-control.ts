...
function parseInteger(value: string, label: string): number {
  if (!/^\d+$/.test(value)) throw new TmuxControlProtocolError(`Invalid ${label}`);
  const parsed = Number(value);
  if (!Number.isSafeInteger(parsed)) throw new TmuxControlProtocolError(`Invalid ${label}`);
  return parsed;
}

function parseGuard(line: Uint8Array, name: "%begin" | "%end" | "%error") {
  const match = ascii(line).match(new RegExp(`^${name} (\\d+) (\\d+) (\\d+)$`));
  if (!match) throw new TmuxControlProtocolError(`Malformed ${name} record`);
  return {
...
export function decodeTmuxControlBytes(encoded: Uint8Array): Uint8Array {
  ...
  for (let index = 0; index < encoded.length; index += 1) {
    ...
    if (byte !== 0x5c) {
      if (byte < 0x20) throw new TmuxControlProtocolError("Unescaped control byte in pane output");
      decoded.push(byte);
      continue;
    }
    if (index + 3 >= encoded.length)
      throw new TmuxControlProtocolError("Truncated octal escape in pane output");
...
function splitAtSpaces(line: Uint8Array, count: number): [string[], Uint8Array] {
  const fields: string[] = [];
  let start = 0;
  for (let index = 0; index < line.length && fields.length < count; index += 1) {
    if (line[index] !== 0x20) continue;
    fields.push(ascii(line.subarray(start, index)));
    start = index + 1;
  }
  if (fields.length !== count) throw new TmuxControlProtocolError("Malformed control record");
...
export class TmuxControlParser {
  ...
  push(chunk: Uint8Array): TmuxControlEvent[] {
    if (this.failed) throw new TmuxControlProtocolError("Parser is in a failed state");
    try {
      if (chunk.byteLength) {
  ...
  private parseLine(line: Uint8Array, events: TmuxControlEvent[]): void {
    if (this.command) {
      if (ascii(line).startsWith("%end ")) {
        ...
        if (this.commandBytes > this.maxRecordBytes)
          throw new TmuxControlProtocolError("Command response exceeds size limit");
    ...
    if (ascii(line).startsWith("%output ")) {
      ...
      assertPaneId(paneId);
      events.push({ type: "output", paneId, data: decodeTmuxControlBytes(payload), lagMs: null });
      return;
    }
    if (ascii(line).startsWith("%extended-output ")) {
      ...
      if (delimiter === -1) throw new TmuxControlProtocolError("Malformed extended output record");
      events.push({
        type: "output",
        paneId,
    ...
    if (/^%(?:begin|end|error)(?: |$)/.test(text))
      throw new TmuxControlProtocolError("Unexpected or malformed command guard");
    const lifecycle = text.match(/^%(pause|continue) (%\d+)$/);
    if (lifecycle) {
      events.push({ type: lifecycle[1] as "pause" | "continue", paneId: lifecycle[2]! });
      return;
    }
    if (text === "%exit" || text.startsWith("%exit ")) {
      events.push({ type: "exit", reason: line.length > 5 ? utf8(line.subarray(6)) : null });
      return;
    }
    ...
    if (notification) {
      ...
      events.push({
        type: "notification",
        name: notification[1]!,
        arguments: argumentOffset < line.length ? utf8(line.subarray(argumentOffset)) : "",
      });
      return;
    }
    throw new TmuxControlProtocolError("Unexpected data outside a command block");
  }

  private closeCommand(line: Uint8Array, success: boolean, events: TmuxControlEvent[]): void {
...
function assertPaneId(paneId: string): void {
  if (!/^%\d+$/.test(paneId)) throw new RangeError("Invalid tmux pane ID");
}
