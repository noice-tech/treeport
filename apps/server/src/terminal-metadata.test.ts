...
import { ProductEventBus, type TaskTTYService, type TmuxAdapter } from "@tasktty/core";
import { TerminalMetadataManager, TERMINAL_METADATA_POLL_MS } from "./terminal-metadata.js";
...
class FakeObserver implements TerminalProgressObserver {
  ...
  progress(progress: Parameters<TmuxProgressObserverOptions["onProgress"]>[0]): void {
...
function fixture(initialTerminals: TerminalRecord[]) {
  ...
  const refreshTerminalStatus = vi.fn(async (terminalId: string) => terminals.get(terminalId)!);
  const service = {
    events,
    database: {
      terminal: (terminalId: string) => terminals.get(terminalId),
      worktree: (worktreeId: string) => (worktreeId === worktree.id ? worktree : undefined),
    },
  ...
  const manager = new TerminalMetadataManager(service, tmux, process.execPath, createObserver);
  return { manager, events, observers, refreshTerminalStatus, sessionTitle, terminals };
}
...
describe("TerminalMetadataManager", () => {
  it("observes every running terminal for the daemon lifetime and publishes metadata", async () => {
    ...
    events.subscribe((event) => {
      if (event.type === "terminal.metadata") published.push(event.data);
    });
  ...
  it("polls unviewed terminals for title/status changes and stops exited observers", async () => {
    ...
    const { manager, observers, refreshTerminalStatus, sessionTitle, terminals } = fixture([item]);
    managers.push(manager);
    await manager.initialize();
    ...
    expect(manager.get("one")).toEqual({ terminalId: "one", title: "finished", progress: null });
  ...
  it("does not revive an observer when an older title lookup finishes after exit", async () => {
    ...
    const initialTrack = manager.trackTerminal(item, worktree);
    await vi.waitFor(() => expect(observers).toHaveLength(1));
    await manager.trackTerminal({ ...item, status: "exited", exitCode: 0 }, worktree);
    resolveTitle("stale running title");
    await initialTrack;

    expect(observers).toHaveLength(1);
    expect(observers[0]!.disposed).toBe(true);
    expect(manager.get(item.id)).toEqual({ terminalId: item.id, title: null, progress: null });
  });
  ...
  it("tracks terminals created after startup and disposes them on removal", async () => {
    ...
    events.publish("terminal.created", { terminalId: created.id, worktreeId: worktree.id });
    await vi.waitFor(() =>
    ...
    events.publish("terminal.removed", { terminalId: created.id, worktreeId: worktree.id });
    expect(manager.snapshot()).toEqual([]);
    expect(observers[0]!.disposed).toBe(true);
  });
});
