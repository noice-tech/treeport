...
if-shell -F '#{==:#{sixel_support},1}' 'set-option -s terminal-features[999] "xterm-256color:hyperlinks:sixel"'
...
export class TmuxAdapter {
  ...
  async createSession(input: {
    ...
    try {
      ...
      try {
        ...
        const imageProtocol = await this.imageProtocol(input.socketName)
        const launchEnvironment: Record<string, string> = {
          ...(sshAuthSock ? { SSH_AUTH_SOCK: sshAuthSock } : {}),
          ...input.env
        }

        delete launchEnvironment.TASKTTY_IMAGE_PROTOCOL
        if (imageProtocol) {
          launchEnvironment.TASKTTY_IMAGE_PROTOCOL = imageProtocol
        }
  ...
  private async imageProtocol(socketName: string): Promise<'sixel' | null> {
    ...
    const [version = '', sixelSupport = ''] = result.stdout.trim().split('\t')
    ...
    return (major > 3 || (major === 3 && minor >= 6)) && sixelSupport === '1'
      ? 'sixel'
      : null
  }
