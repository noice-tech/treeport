...
describe("TerminalProgressParser", () => {
  ...
  it("extracts OSC 9;4 updates terminated by BEL or ST", () => {
    ...
    expect(parser.push(bytes("\x1b]9;4;1;42\x1b\\"))).toEqual([{ state: "normal", value: 42 }]);
    expect(parser.push(bytes("\x9d9;4;2;100\x9c"))).toEqual([{ state: "error", value: 100 }]);
  ...
  it("ignores unrelated, malformed, and oversized OSC payloads and then recovers", () => {
    const parser = new TerminalProgressParser();
    expect(
      parser.push(bytes(`\x1b]0;title\x07\x1b]9;4;1;101\x07\x1b]${"x".repeat(80)}\x1b]9;4;3\x07`)),
