...
describe("TerminalAttachmentManager", () => {
  it("reapplies tmux server configuration before reading or attaching the session", async () => {
    ...
    manager.message(manager.accept("term", socket as unknown as WSContext), hello("tab-a"));
    await ready(socket);
    expect(tmux.configureServer).toHaveBeenCalledWith("socket");
    expect(vi.mocked(tmux.configureServer).mock.invocationCallOrder[0]).toBeLessThan(
      vi.mocked(tmux.sessionSize).mock.invocationCallOrder[0]!,
    );
    expect(vi.mocked(tmux.configureServer).mock.invocationCallOrder[0]).toBeLessThan(
      vi.mocked(tmux.attachArgs).mock.invocationCallOrder[0]!,
    );
  });
  ...
  it("rejects oversized frames before parsing", () => {
    ...
    expect(socket.sent.at(-1)).toMatchObject({ type: "error", code: "MESSAGE_TOO_LARGE" });
  ...
  it("preserves binary input bytes for the controller", async () => {
    ...
    manager.message(
      id,
      JSON.stringify({ version: TERMINAL_PROTOCOL_VERSION, type: "binary", data: "\0ÿ" }),
    );
    expect(Buffer.isBuffer(ptys[0]!.writes[0])).toBe(true);
    expect((ptys[0]!.writes[0] as Buffer).equals(Buffer.from([0, 255]))).toBe(true);
    manager.close(id);
  });
  ...
  it("does not expose the reconnect credential in controller events", async () => {
    ...
    manager.message(
      viewerId,
      JSON.stringify({ version: TERMINAL_PROTOCOL_VERSION, type: "take_control" }),
    );
  ...
  it("restores controller ownership to the same tab during reconnect grace", async () => {
    ...
    const reconnectId = manager.accept("term", reconnect as unknown as WSContext);
    manager.message(reconnectId, hello("tab-a"));
    await ready(reconnect);
    const readyMessage = reconnect.sent.find((message) => message.type === "ready");
