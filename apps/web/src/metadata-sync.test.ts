...
describe("metadata sync", () => {
  it("retries transient failures within a fixed bound", () => {
    ...
    expect(shouldRetryMetadataQuery(1, new ApiError("UPSTREAM", "bad", 503))).toBe(true);
    expect(shouldRetryMetadataQuery(0, new ApiError("UNAUTHORIZED", "no", 401))).toBe(false);
