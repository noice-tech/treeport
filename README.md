...
A real terminal BEL from a terminal that is not actively visible uses a native OS notification in packaged Electron. Development builds, and packaged builds where native notification delivery is unavailable or reports a failure, keep a persistent in-app BEL toast and request OS-level window attention instead. **View** restores TaskTTY and opens the exact terminal, while **Dismiss** acknowledges that BEL without navigating. Native notification action buttons are best effort across operating systems, and body click is the portable View action. Delivery requires the Electron process and renderer to remain running and is still subject to OS notification permissions and focus policy. Production macOS delivery requires signing, while installed Windows delivery needs application identity and shortcut integration.
