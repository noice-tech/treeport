...
An HTTP application loaded in the Browser package's nested iframe can use `treeport.panel.setTitle()` and `treeport.shortcuts.onFind()`. Numbered `Cmd+1` through `Cmd+9` workspace shortcuts are also routed from a target that includes the SDK. The desktop app captures its numbered workspace shortcuts before the target receives them, so that path does not require the SDK. The Browser package relays only these title and shortcut messages. The target must include the SDK in its own application build for browser-client routing. Browser targets cannot use context, diff, network discovery, storage, launch, or external URL methods.
