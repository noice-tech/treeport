...
function writable() {
  let value = "";
  return {
    stream: { write: (chunk: string | Uint8Array) => ((value += String(chunk)), true) },
    value: () => value,
  };
}
...
describe("terminal launcher setup pipeline", () => {
  it("runs setup tasks sequentially before the exact final argv", async () => {
    ...
    const results = [0, 0, 7];
    const spawnProcess = vi.fn((executable: string, args: readonly string[], options: unknown) => {
    ...
    const code = await runLaunchSpec(
      spec({
        setupTasks: [
          { label: "first", argv: ["one", "a b"], cwd: "/one", env: { ONE: "1" }, timeoutMs: 100 },
          {
            label: "second",
            argv: ["two", "$HOME"],
            cwd: "/two",
  ...
  it("stops on the first setup failure and does not start the final command", async () => {
    ...
    const code = await runLaunchSpec(
      spec({
        setupTasks: [
          { label: "bad", argv: ["false"], cwd: "/worktree", env: {}, timeoutMs: 100 },
          { label: "skipped", argv: ["echo"], cwd: "/worktree", env: {}, timeoutMs: 100 },
        ],
      }),
      {
        spawnProcess,
  ...
  it("forwards termination signals to the active child", async () => {
    const child = new FakeChild();
    const spawnProcess = vi.fn(() => child as unknown as ChildProcess) as unknown as typeof spawn;
  ...
  it("does not continue when a setup child handles a forwarded signal and exits cleanly", async () => {
    ...
    const launch = runLaunchSpec(
      ...
      { spawnProcess, signalSource, stderr: error.stream, stdout: writable().stream },
    );
    await vi.waitFor(() => expect(spawnProcess).toHaveBeenCalledTimes(1));
  ...
  it("times out a setup task and skips the final command", async () => {
    ...
    const code = await runLaunchSpec(
      spec({
        setupTasks: [{ label: "slow", argv: ["sleep"], cwd: "/worktree", env: {}, timeoutMs: 1 }],
      }),
      {
        spawnProcess,
        stderr: error.stream,
