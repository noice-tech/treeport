...
async function assertNoSymlinkComponents(parent: string, candidate: string): Promise<void> {
  if (!isPathWithin(candidate, parent))
    throw new Error("Zed worktree path must stay inside the repository or its parent");
  const relative = path.relative(parent, candidate);
  let current = parent;
  for (const component of relative.split(path.sep).filter(Boolean)) {
    ...
    if (stat.isSymbolicLink())
      throw new Error(`Zed worktree path cannot contain a symbolic link: ${current}`);
  }
}
...
export function normalizeWorktreeName(input: string): string {
  ...
  if (!name || name === "." || name === "..") throw new Error("Worktree name is required");
  ...
  if (
    ...
    throw new Error("Worktree name cannot contain path separators or control characters");
  return name;
}
...
export async function resolveZedWorktreePath(
  mainWorktreePath: string,
  inputName: string,
): Promise<{ name: string; path: string; wrapperPath: string; directorySetting: string }> {
  const name = normalizeWorktreeName(inputName);
  const settings = await readJsonc(path.join(mainWorktreePath, ".zed", "settings.json"));
  const record =
    settings && typeof settings === "object" ? (settings as Record<string, unknown>) : {};
  const git =
    record.git && typeof record.git === "object" ? (record.git as Record<string, unknown>) : {};
  ...
  if (!directorySetting || directorySetting === ".." || path.isAbsolute(directorySetting))
  ...
  if (resolved === parent || !isPathWithin(resolved, parent))
    throw new Error("Zed git.worktree_directory must stay inside the repository or its parent");
...
export async function prepareZedWorktreeWrapper(
  ...
  try {
    ...
    if (stat.isSymbolicLink())
      throw new Error(`Zed worktree wrapper cannot be a symbolic link: ${wrapper}`);
    if (!stat.isDirectory()) throw new Error(`Zed worktree wrapper is not a directory: ${wrapper}`);
  } catch (error) {
...
export async function loadCreateWorktreeTasks(mainWorktreePath: string): Promise<ZedTask[]> {
  const value = await readJsonc(path.join(mainWorktreePath, ".zed", "tasks.json"));
  return taskArray(value).flatMap((entry, index) => {
    if (!entry || typeof entry !== "object") return [];
    const task = entry as Record<string, unknown>;
    if (!Array.isArray(task.hooks) || !task.hooks.includes("create_worktree")) return [];
    if (typeof task.command !== "string" || !task.command.trim())
      throw new Error(`Zed create_worktree task ${index + 1} is missing a command`);
    ...
    const args = (task.args ?? []).map((argument) => {
      if (typeof argument !== "string")
        throw new Error(`Zed create_worktree task ${index + 1} has a non-string argument`);
      return argument;
    });
    const env: Record<string, string> = {};
    if (task.env !== undefined) {
      if (!task.env || typeof task.env !== "object" || Array.isArray(task.env))
      ...
      for (const [key, environmentValue] of Object.entries(task.env)) {
        if (typeof environmentValue !== "string")
          throw new Error(`Zed create_worktree task ${index + 1} has a non-string env value`);
    ...
    return [
      {
        label:
          typeof task.label === "string" && task.label.trim() ? task.label : `Task ${index + 1}`,
...
export async function runCreateWorktreeTasks(input: {
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      results.push({
        label: task.label,
        error: (error instanceof Error ? error.message : String(error)).slice(0, MAX_HOOK_OUTPUT),
      });
      break;
    }
  }
