...
function defaultDataDir(env: NodeJS.ProcessEnv, platform = process.platform): string {
  if (env.XDG_DATA_HOME) return path.join(expandHome(env.XDG_DATA_HOME), "tasktty");
  if (platform === "darwin")
...
function defaultRuntimeDir(env: NodeJS.ProcessEnv): string {
  if (env.XDG_RUNTIME_DIR) return path.join(expandHome(env.XDG_RUNTIME_DIR), "tasktty");
  return path.join(
    os.tmpdir(),
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  const dataDir = path.resolve(expandHome(env.TASKTTY_DATA_DIR || defaultDataDir(env)));
  const runtimeDir = path.resolve(expandHome(env.TASKTTY_RUNTIME_DIR || defaultRuntimeDir(env)));
  const authToken = env.TASKTTY_AUTH_TOKEN?.trim() || null;
  if (host !== "127.0.0.1" && host !== "::1" && host !== "localhost" && !authToken) {
    throw new Error("Refusing to bind a non-loopback address without TASKTTY_AUTH_TOKEN");
  }
  return {
    host,
    port: portValue,
    ...
    shell: path.resolve(expandHome(env.TASKTTY_SHELL || env.SHELL || "/bin/sh")),
