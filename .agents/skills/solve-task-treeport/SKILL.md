...
Do not implement the task in the coordinating worktree. Create a separate Treeport-managed worktree and start a persistent interactive Pi agent there with the task as its initial prompt.
...
Use `--from-current` only when the task must start from the current worktree's committed `HEAD`; uncommitted changes are not copied. Otherwise use Treeport's default base.
